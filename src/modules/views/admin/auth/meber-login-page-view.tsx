...
export const MemberLoginPageView = ({
  invitationId,
  memberEmail,
  organizationId,
}: Props) => {
  ...
  const signupSchema = z.object({
    ...
    organizationId: z.string().min(1, {
      message:
        "Organization Id is missing. Please click the invitation link again!",
    }),
    invitationId: z.string().min(1, {
      message:
        "Invitation Id is missing. Please click the invitation link again!",
    }),
  });
  ...
  const sendOTPMutation = useMutation({
    mutationFn: async (data: SignupSchemeType) => {
      ...
      const response = await axiosInstance.post("/auth/send-member-otp", data);
  ...
  const verifyEmailMutation = useMutation({
    mutationFn: async (data: VerifyEmailType) => {
      const response = await axiosInstance.post(
        "/auth/verify-member-email",
        data
      );
      return response.data;
    },
    onSuccess: (data) => {
      const memberData: UserPros = {
        id: data.data.id,
        name: data.data.name,
          ...
          <Form {...form}>
            ...
            >
              ...
              <p className="text-sm text-neutral-600 text-center">
                Already have an member account?{" "}
                <Link
                  href="/login"
              ...
              <p className="text-sm text-neutral-500 text-center">
                Create your member account to get started with your
                organization.
              </p>
            </form>
          </Form>
